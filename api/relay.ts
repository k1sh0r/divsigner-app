...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    // The controller/timeoutId live in the loop body so the catch branch
    // can clear the timer when fetch throws synchronously (otherwise the
    // timer fires later into a no-op abort, but the closure leaks).
...
export default async function handler(request: Request): Promise<Response> {
  // CORS: echo the request origin (with Vary: Origin so caches don't
  // mix responses across origins). Credentials are not used — the API key
  // travels in the Authorization/x-api-key header — so echoing any origin
  // is safe. The dev server still hardcodes `*`, and production must NOT
  // do the same because `*` would block `Access-Control-Allow-Credentials`
  // if it were ever enabled. For prod, only the configured allowlist is
  // echoed; unknown origins fall through to the first allowed origin so
  // the browser can still see the response (and show its own error).
  ...
  try {
    ...
    // SSE responses must not be cached — Vercel/Netlify edge caches have
    // bitten us before on a 200 chunked response that was replayed as a
    // 200 already-complete response on the next request.
